...
and the number (and quality) of medial staff

Staff shortages in the health care (HC) system are currently a serious
barrier to the implementation of proper medical care in Poland and
Europe

Staff shortages in HC can be attributed to several factors:
**aging population**, **personel burnout**, but also:

* improper management of human resources,

* failure to adjust the **employment structure** to health needs,

* a defective education system based on public universities monopoly

* HC system underfunding

There are also common public opinions (aka myths) on this problem:

* Staff shortages are particularly acute in Poland

* Staff shortages are caused by mass abroad migration (of doctors)
...
Establish the basic facts and debunk public myths

## Material and method

Data on the number of doctors/medical graduates come from Eurostat (tables `hlth_rs_phys`, `hlth_rs_grd` and `hlth_rs_wkmg`).
...
Additionally selected information contained in reports and scientific publications on the subject was used.

## Number of doctors in EU

We use [number of] **people per 1 physician** rate (last reported year; 2017 for PL)

EU member states were divided into two groups:

* **new member states** (NEU), 
  ie. Czech Rep., Estonia, Lithuania, Latvia, Poland, Slovakia, Slovenia, Hungary, Bulgaria, Romania and Croatia.

* **old member states** (OEU), Austria, Belgium, Denmark, Finland, 
France, Germany, Greece, Ireland, Italy, Netherlands, Portugal,
Spain, Sweden.

Cyprus, Malta and Luxembourg were omitted.
...
(`r sprintf("%.1f", median.tt.neu)` vs `r sprintf("%.1f", median.tt.oeu)`)

Poland has the worst  value of the rate (`r sprintf("%.1f", wtt.pl.last)`),
which is `r sprintf ("%.1f", wtt.pl.last/median.tt.neu * 100)`% of the
...
EU countries) is `r sprintf("%.1f", level.tt)`.

The median of the average change in the reporting period is `r sprintf("%.2f", change.tt)`.
(in half of the countries, the value of the indicator 
drops not less than `r sprintf("%.2f", change.tt)` )

The value of the indicator for Poland is `r sprintf("%.1f", level.tt.pl)` and
is `r sprintf ("%.1f", level.tt.pl/level.tt * 100 - 100 )`% higher than the median value;

The average change in the reporting period in Poland is `r sprintf("%.2f", change.tt.pl)`
...
of the median for the analyzed group of countries (`r sprintf("%.2f", change.tt)`).


## Elderly people per 1 physician

Due to the aging of the population it is worth analyzing the ratio
of the number of **elderly people per 1 physician** (ie. people aged 65+ per 1 physician).
...
On the average, new EU countries have higher rates
(median equal to `r sprintf("%.1f", median.pp.neu)` than old ones (`r sprintf("%.1f", median.pp.oeu)`)

Poland has again the worst rate value (`r sprintf("%.1f", w65.pl.last)`)
...
The median of the last reported value is `r sprintf("%.1f", level65)`.

The median of the average change in the reporting period is `r sprintf("%.3f", change65)`.
(in half of the countries, the rate value increases on average
no more than `r sprintf("%.1f", change65)` )

The value of the indicator for Poland is `r sprintf("%.1f", level65.pl)`
and is `r sprintf ("%.1f", level65.pl/level65 * 100 - 100)`% higher than the median value;

The average change in the reporting period is `r sprintf("%.1f", change65.pl)` 
and is `r sprintf ("%.1f", change65.pl/change65 * 100 - 100)`% higher than the median


## Graduates of medical studies

Medical graduates are future doctors. 
Therefore, the **population per 1 graduate** rate determines the size of the stream
supplying the HC system with new staff.
...
On the average, there were more medical graduates in the new EU
countries. Poland is next to last (again), but many countries with a better
reputation for HC have relatively high values (Germany, Sweden, Finland) as well.
...
The median population per graduate ratio in the group of new EU members is `r sprintf ("%.0f", graduates.nue.mean)` (Poland `r sprintf ("%.0f", graduates.pl.gpc)` or `r sprintf ("%.2f", coeff.g.pl * 100)`%).

In the last reported year, there were `r graduates.pl.g` 
graduates in Poland. To achieve the average for new EU members
it would be necessary to educate `r sprintf ("%.0f", pl.mean.neu)`, i.e. about `r sprintf ("%.0f", pl.mean.neu.diff)` more (increase by `r sprintf ("%.1f", pl.mean.neu.diff/graduates.pl.g * 100 )`% ). Pretty formidable task.
...
The number of people aged 65+ in Poland 
will systematically grow until 2062, reaching over 10 million.

The share of people aged 65+in Europe will increase.
The median for the analyzed group of countries in 2023 will be `r sprintf("%.1f",mean.pp.2023)`
and in 2048 it will be `r sprintf("%.1f",mean.pp.2048)`

In 2017, the total population of Poland was
`r pop2017pl.tt `, population aged 65+ was 
...
## Increase in the number of doctors in the HC system
...
Steady increase in the analyzed period is observed. 
The approximation with linear trend showed that, the average annual increase
...
1. The per capita rate [of doctors] in the 65+ group will be equal to
the average in the group of new EU members, adjusted by the average
annual change

2. The per capita rate [of doctors] in the 65+ group will be equal to
the average in the whole EU, adjusted by the average annual change

3. The total per capita rate [of doctors] will be equal to the average
in the group of new EU members, adjusted by the average annual change

4. The total per capita rate [of doctors] will be equal to the average
in the whole EU, adjusted by the average annual change
...
The median was taken as the mean value.
It was assumed that the observed average change in this average value will continue in the forecast period.
For example, if in the group of new EU countries, the average annual change was `r sprintf("%.1f", change65.neu)`
we assume that in 2048 this average will be `r sprintf("%.1f", change65.neu * (2048 - 2017))` people per doctor higher (annual change multiplied by 31, i.e. the difference between 2048 and 2017).

Of course, this is a very simplified assumption and it boils down
to determining what will happen if the observed trends do not change.

The rapid aging of the population justifies the use of the population 65+/doctor rate, 
not the total population rate.
...
the one observed in years `r pl.yr.first`--`r pl.yr.last` (i.e. `r sprintf("%.1f", b.coeff)` doctors per year) are presented at the chart below
...
There is no data on the number of graduates of medical faculties of Polish medical schools, 
but the Center for Medical Examinations (CEM)
...
Practically no increase is observed in the analyzed period (on the average, 
there are extra `r b.coeff` graduates per year.)

Conclusion1: with the current system of education, there is no chance for a greater influx of doctors to the system.

Conclusion2: the low dynamics of the number of graduates entering 
the profession contradicts the high dynamics of graduates (according to Eurostat)
...
this share increases slightly (in 15 years by less than 5%, practically from zero)
