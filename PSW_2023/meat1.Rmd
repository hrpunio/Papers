...
* daily per capita energy supply (kcal/c/d or **kilocalories/per capita/daily**),
...
BTW: https://www.youtube.com/watch?v=2-LunSug43 (How to pronounce Kuznets)
...
of Animal Products at  `r sprintf ("%.1f", apex)` (thousand) USD/pc. 

This is very high value but is consistent with **Cole J.R. and McCoskey S. (2017)**
who using data for 2009 reported 43,9 thousand USD/pc.
...
```{r}
konf.part <- c(
  '616', '012', '268', '800', '400', '504', '792', '356', '417', '100',
  '275', '498', '788', '051', '008', '688' )

o0od2023 <- o0 %>% filter (CodeM49 %in% konf.part )  %>%
  filter (Item == 'Meat' & Element =='FoodSQ (kg/c/y)' ) %>%
  mutate (V2020 = as.character(Value))

o0od2023$V2020[ (o0od2023$YearCode == "2010" ) ] <- ""

od2023.cons.p <- ggplot(o0od2023, aes(x= Area,  y=Value, fill=as.factor(YearCode) )) +
  geom_bar(stat='identity', position="identity",  alpha=.3) +
  xlab(label="") +
  ylab("kg/capita/year") +
  theme(legend.position = "top") +
  labs(fill="") +
  geom_text(aes(label = sprintf ("%s", V2020)), size=3, color='red', hjust=1.25) +
  ggtitle('Consumption of meat in countries represented at OD2023 (2020 vs 2010)') +
  coord_flip()
od2023.cons.p
